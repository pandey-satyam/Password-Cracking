//Referring Sample Solution given
<?php
$error = false;
$md5 = false;
$code = "";
if ( isset($_GET['code']) ) {
    ...
    if ( strlen($code) != 4 )
    {
        $error = "input has to exactly 4 numbers";
    }
    else if ( $code[0] < "0" || $code[0] > "9" || $code[1] < "0" || $code[1] > "9" ||$code[2] < "0" || $code[1] > "9" ||$code[3] < "0" || $code[1] > "9" )
    {
        $error = "input range should be 4 lower numbers";
    }
    else
    {
        $md5 = hash('md5', $code);
...
?>
<!DOCTYPE html>
<head><title>Satyam Pandey PIN Code</title></head>
<body>
<h1>MD5 PIN Maker</h1>
<?php
if ( $error !== false ) {
